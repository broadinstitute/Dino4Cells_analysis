...
from utils.augmentations import Single_cell_Resize, Single_cell_centered
...
class ViTExtractor:
    ...
    def preprocess(
        ...
        if img.shape[0] == 7:
            img = img[[1, 2, 3], :, :].transpose(1, 2, 0)
        # img = io.imread(image_path).transpose(1, 2, 0)[:, :, [0, 1, 2, 3]].astype(float)
        img -= img.min()
        img /= img.max()
    ...
    def _get_hook(self, facet: str):
        ...
        def _inner_hook(module, input, output):
            ...
            qkv = (
                module.qkv(input)
                .reshape(
                    #     B, N, 4, module.num_heads, C // module.num_heads
                    # )
                    B,
                    N,
                    3,
